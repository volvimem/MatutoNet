...
// =========================================================================
// 1. SISTEMA DE AUTENTICAÇÃO E ISOLAMENTO DE DADOS (O CORE DO SaaS)
// =========================================================================

// Escuta quando o usuário entra ou sai
onAuthStateChanged(auth, (user) => {
    if (user) {
        // Se entrou: Esconde o login, mostra o app e liga o banco
        document.getElementById('telaLogin').style.display = 'none';
        document.getElementById('sistemaApp').style.display = 'block';
        iniciarBancoDeDados(user.uid);
    } else {
        // Se saiu: Esconde o app, mostra o login e tranca tudo
...
function iniciarBancoDeDados(uid) {
    // Aponta o banco para a pasta desse usuário específico
    ...
    onValue(refConfig, snap => { const c = snap.val() || {}; chavePixGlobal = c.chavePix || "Não configurada"; document.getElementById('chavePixConfig').value = c.chavePix || ""; document.getElementById('diasFaturaConfig').value = c.diasFatura || 5; });
}

function trancarPortasDoBanco() {
    // Desliga as escutas do banco se o usuário sair
    dadosClientes = {}; dadosHistorico = {};
    if(refClientes) off(refClientes); if(refHistorico) off(refHistorico); if(refConfig) off(refConfig);
}

// O RESTO DO SISTEMA (CLIENTES, DASHBOARD, IMPRESSÃO, PIX) CONTINUA IGUAL...
// ... apenas substituí 'ref(db, ...)' por 'refClientes', 'refHistorico' e 'refConfig' ...

document.getElementById('formConfig').addEventListener('submit', function(e) { e.preventDefault(); const pix = document.getElementById('chavePixConfig').value.trim(); const dias = parseInt(document.getElementById('diasFaturaConfig').value) || 5; set(refConfig, { chavePix: pix, diasFatura: dias }).then(() => { Swal.fire('Sucesso!', 'Configurações salvas e aplicadas ao Robô!', 'success'); fecharModalConfig(); }); });
